...
func GetDocForId(fileLineType string, metaData VxMetadata, headerData []string, dataData []string, dataKey string) (interface{}, error) {
	...
	case "STAT_VAL1L2":
		...
		doc["Type"] = metaData.Type
		doc["SubType"] = metaData.SubType
		// check if data exists in doc and if not create it - this is a new doc so it should not exist
		if exists := (doc)["data"]; exists == nil {
	...
	// remove  the items in doc that have "" as a value
	for k, v := range doc {
		if v == "" {
			delete(doc, k)
		}
...
// data key definitions -  these may need to be changed based on the data and how we want to associate records into a single document
